...
PetscErrorCode read_data(PetscInt nc, PetscInt ngen, PetscInt nload, PetscInt nbus, PetscInt nbranch, Mat Ybus,Vec V0,Gen **pgen,Load **pload,Bus **pbus, Branch **pbranch, PetscInt **pedgelist)
{
  ...
  */
  ...
  ierr = PetscCalloc4(nbus*nc,&bus,ngen*nc,&gen,nload*nc,&load,nbranch*nc,&branch);CHKERRQ(ierr);
 ...
 ierr = PetscCalloc1(2*nbranch*nc,&edgelist);CHKERRQ(ierr);
...
>>>>>>> 0b520e273a8875e4acc4319eff612b23290c06ce
 ...
 // ierr = MatDestroy(&Ybus);CHKERRQ(ierr);
  ...
  ierr = DMNetworkSetSizes(networkdm,nbus*nc,nbranch*nc,PETSC_DETERMINE,PETSC_DETERMINE);CHKERRQ(ierr);
 ...
 //   user.tmax      = 5.0;
	...
	for (i=0; i<nc; i++){
    user.ybusfault[user.faultbus[i]*2+1]=1/user.Rfault;
	}
  }
