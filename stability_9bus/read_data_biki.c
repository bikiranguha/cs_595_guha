...
PetscErrorCode ReadData(SYSDATA *pgrid, Num Sys_num/* fill up along the way */)
 {
	 // Define all the temporary variables here
	 PetscInt i,j;
  ...
  pgrid->nbus = Sys_num->nbus;
  pgrid->ngen = Sys_num->ngen;
  pgrid->nbranch = Sys_num->nbranch;
  pgrid->nload = Sys_num->nload; 
	 
	 
	 
	 // The actual reading starts here
	 
  ierr = PetscCalloc1(pgrid->nbus,&pgrid->bus);CHKERRQ(ierr);
  ierr = PetscCalloc1(pgrid->ngen,&pgrid->gen);CHKERRQ(ierr);
  ierr = PetscCalloc1(pgrid->nload,&pgrid->load);CHKERRQ(ierr);
  ierr = PetscCalloc1(pgrid->nbranch,&pgrid->branch);CHKERRQ(ierr);
	 
	 
	 /* Filling up bus numbers */
	 for(i=0; i < pgrid->nbus; i++) {
    pgrid->bus[i].bus_i=i;
  }
   /* Initializing the ngen and nload values of each bus data */
   for(i=0; i < pgrid->nbus; i++) {
    pgrid->bus[i].ngen= pgrid->bus[i].nload = 0;
  }
  
  
  /* Identifying which buses have generators and/or loads */
  for (i=0; i < pgrid->nbus; i++){
	  for (j=0; j<3; j++){
		
		if(pgrid->bus[i].bus_i == gbus[j]){
			
			pgrid->bus[i].ngen = 1;
		}
		
		if (pgrid->bus[i].bus_i== lbus[j]){
